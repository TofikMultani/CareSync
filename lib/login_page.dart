...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _signInWithGoogle() async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.message ?? 'Google Login failed'), backgroundColor: Colors.red));
    } catch (e) {
      if (mounted) {
        showDialog(
          context: context,
          builder: (_) => AlertDialog(
            title: const Row(children: [Icon(Icons.warning, color: Colors.orange), SizedBox(width: 8), Text("Auth Configuration")]),
            content: Text("Google Sign-In failed. If you are on Windows or Web, please ensure you have configured OAuth Client IDs. On Android, verify SHA-1 fingerprints in Firebase Console.\n\nError details: $e"),
            actions: [
              TextButton(onPressed: () => Navigator.pop(_), child: const Text("OK", style: TextStyle(color: Colors.teal)))
            ],
          ),
        );
      }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Icon(Icons.health_and_safety, size: 50, color: Colors.teal.shade700),
                      ),
                      ...
                      TextButton(
                          ...
                          try {
                            ...
                            if(mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                                content: Text('Admin created! Login with admin@healthcare.com / adminpassword'),
                        ...
                        child: Text("Developer: Create Default Admin Account", style: TextStyle(color: Colors.grey.shade400, fontSize: 11)),
                      ),
                    ],
                  ),
                ),
